...
import { IconBuildingBank, IconBlockquote, IconNetwork, IconClock } from '@tabler/icons-react';
...
export default function OverViewLayout({
  ...
  // Compute aggregated blockchain metrics
  const totalChains = chainConfigs.length;
  const avgHeight = Math.round(
    chainConfigs.reduce((sum, cfg) => sum + (chainData[cfg.display]?.height || 0), 0) /
    totalChains
  );
  const totalPeers = chainConfigs.reduce(
    (sum, cfg) => sum + (chainData[cfg.display]?.peer_count || 0),
    0
  );
  const totalUnconfirmed = chainConfigs.reduce(
    (sum, cfg) => sum + (chainData[cfg.display]?.unconfirmed_count || 0),
    0
  );
  ...
  return (
    <PageContainer>
      <div className="flex flex-1 flex-col space-y-4">
        ...
        {/* Stats Cards (blockchain overview) */}
        <Infocards chainConfigs={chainConfigs} chainData={chainData} />
